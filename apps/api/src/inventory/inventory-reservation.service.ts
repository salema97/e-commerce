  ...
  async reserveItems(items: ReservationItem[]): Promise<void> {
    await this.prisma.$transaction(async (tx) => {
      for (const item of items) {
        ...
        if (!inv) throw new BadRequestException(`No inventory for product ${item.productId}`);
  ...
  async releaseExpiredReservations(before = new Date()): Promise<number> {
    ...
    for (const { id } of expired) {
      ...
      await this.prisma.order.update({
        where: { id },
        data: { status: OrderStatus.CANCELLED, statusHistory: { create: { status: OrderStatus.CANCELLED, notes: 'Reservation expired' } } },
      });
    }
